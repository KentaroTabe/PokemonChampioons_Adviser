...
**自分のチームの型登録 (推奨)**: 画面からは自分の能力ポイント・性格は読み取れない
ため、未登録だと攻撃系252振りを仮定します。`config/my_team.example.json` を
`config/my_team.json` にコピーして自分の型を書くと、ダメージ計算・素早さ比較が
実際の型で行われます (能力ポイントは0-32のゲーム内表記でOK。サーバー再起動不要、
ファイル保存で即反映)。未登録のポケモンには画面の📱提案で登録を促します。
