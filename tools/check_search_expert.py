...
async def run(n_battles: int, depth: int, by: str = "recommended",
              ...
              sensor_delta: float = 0.25, workers: int = 1,
              sensor_estimate: bool = False, belief_updates: bool = False) -> None:
    import json
    import random
    ...
    if json_out:
        ...
        Path(json_out).write_text(json.dumps({
            ...
            "sensor_estimate": sensor_estimate, "belief_updates": belief_updates,
...
def main() -> None:
    ...
    ap.add_argument("--sensor-estimate", action="store_true",
                    help="固着中に相手の技イベントから期待ダメージを引く (P10)")
    ap.add_argument("--belief-updates", action="store_true",
                    help="相手型の仮説分布を対戦中の観測 (先後/被ダメ/持ち物) で更新 (P7')")
    args = ap.parse_args()
    asyncio.run(run(args.battles, args.depth, args.by,
                    ...
                    sensor_estimate=args.sensor_estimate,
                    belief_updates=args.belief_updates))
